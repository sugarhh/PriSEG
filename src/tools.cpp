...
string which_network(string network)
{
	if (network.find("SecureML") != std::string::npos)
		...
		return "VGG16";
	else if (network.find("U2netNet") != std::string::npos)
		return "U2netNet";
	else 
...
void print_vector(RSSVectorMyType &var, string type, string pre_text, int print_nos, string layname, bool print_to_console)
{
    size_t temp_size = var.size();
    vector<myType> b(temp_size);
    funcReconstruct(var, b, temp_size, "anything", false);
	...
	//std::string output_dir= "files/preload/U2netNet/text";
	std::string output_dir= "files/preload/U2netNet/all_output";
	// Print to file
    // if (!output_dir.empty()) {
        // Create the output directory if it doesn't exist

        // Generate the output file name
        string output_file = output_dir + "/" + layname + pre_text + ".txt";

        ofstream output_stream(output_file);
        for (int i = 0; i < print_nos; ++i) {
			float oupt = (static_cast<int64_t>(b[i])) / (float)(1 << FLOAT_PRECISION);
			// //if (oupt > 1)	oupt = 1;
			// if (oupt > 1000)	{cout<<"!!!"<<layname << " " << oupt <<endl; break;}
			// else if (oupt < -1000)		{cout<<"!!!"<<layname << " " << oupt <<endl;	break;}
            output_stream << oupt << " ";
            if (i % 32 == 31)
                output_stream << endl;
        }
...
void dividePlain(int divisor, vector<myType> &vec)
{
	assert((divisor != 0) && "Cannot divide by 0");
	
	if (BIT_SIZE == 32)
		for (int i = 0; i < vec.size(); ++i){
			double temp = (myType)((double)((int32_t)divisor)/(double)(vec[i])); 
			vec[i] = (myType)temp;
		}

	if (BIT_SIZE == 64)
		for (int i = 0; i < vec.size(); ++i){
			double temp = (myType)((double)((int64_t)divisor)/(double)(vec[i])); 
			vec[i] = temp;
		}
...
void multiplyByScalar(const RSSVectorMyType &a, float scalar, RSSVectorMyType &b)
{
	size_t size = a.size();
	for (int i = 0; i < size; ++i)
	{

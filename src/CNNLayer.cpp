...
void CNNLayer::forward(const RSSVectorMyType& inputActivation)
{
    ...
    //size_t D = conf.dilation;  // Assuming 'conf' has a 'dilation' field.
	size_t D =  P;
	
    // Output dimensions calculation
    size_t ow = ((iw - (f - 1) * D - 1 + 2 * P) / S) + 1;
    size_t oh = ((ih - (f - 1) * D - 1 + 2 * P) / S) + 1;

	/*
	for (size_t ll = 0; ll < Din; ++ll) {
		RSSVectorMyType temp(iw * ih * 1 * B, make_pair(0, 0));
		for (size_t i = 0; i < B; ++i) {
			for (size_t j = 0; j < oh; ++j) {
				for (size_t k = 0; k < ow; ++k) {
					temp[i * oh * ow + j * ow + k] = inputActivation[i * Din * oh * ow + ll * oh * ow + j * ow + k];
				}
			}
    	}

    	RSSVectorMyType temp1((iw + 2 * P) * (ih + 2 * P) * 1 * B, make_pair(0, 0));
    	zeroPad(temp, temp1, iw, ih, P, 1, B);

    	RSSVectorMyType temp2((f * f * 1) * (ow * oh * B));

    	// Perform convolution
    	for (size_t i = 0; i < B; ++i) {
        	for (size_t j = 0; j < oh; ++j) {
            	for (size_t k = 0; k < ow; ++k) {
                	size_t loc_output = (i * ow * oh + j * ow + k);
                	for (size_t a = 0; a < f; ++a) {
        	            for (size_t b = 0; b < f; ++b) {
            	            size_t input_index = (i * (iw + 2 * P) * (ih + 2 * P) * 1) + (j * S * (iw + 2 * P) + k * S) + a * D * (iw + 2 * P) + b * D;
                	        size_t output_index = ((a * f + b) * ow * oh * B + loc_output);
                    	    if (output_index < temp2.size() && input_index < temp1.size()) {
                        	    temp2[output_index] = temp1[input_index];
							 }
                        }
                    }
                }
    	...
    	RSSVectorMyType wei(Dout * f * f, make_pair(0, 0));
    	for (size_t o = 0; o < Dout; ++o) {
			for (size_t ft = 0; ft < f * f; ++ft){
				wei[o * f * f + ft] = weights[o * Din * f * f + ll * f * f + ft];
			}
    	}

    	RSSVectorMyType temp3(Dout * (ow * oh * B));
    	funcMatMul(wei, temp2, temp3, Dout, (f * f * 1), (ow * oh * B), 0, 0, FLOAT_PRECISION);
    	...
    	for (size_t i = 0; i < B; ++i) {
        	for (size_t j = 0; j < Dout; ++j) {
            	for (size_t k = 0; k < tempSize; ++k) {
                	activations[i * Dout * tempSize + j * tempSize + k] = activations[i * Dout * tempSize + j * tempSize + k] + temp3[j * B * tempSize + i * tempSize + k];
            	}
        	}
    	}
	}
	...
	for (size_t i = 0; i < B; ++i) {
		for (size_t j = 0; j < Dout; ++j) {
			for (size_t k = 0; k < tempSize; ++k) {
				activations[i * Dout * tempSize + j * tempSize + k] = activations[i * Dout * tempSize + j * tempSize + k] + biases[j];
			}
		}
	}
	*/
	...
	// print_vector(temp1, "FLOAT", "temp1:", 49*3);
        ...
        for (size_t j = 0; j < oh; ++j) {
            for (size_t k = 0; k < ow; ++k) {
                ...
                for (size_t l = 0; l < Din; ++l) {
                    size_t loc_input = (i * (iw + 2 * P) * (ih + 2 * P) * Din) + (l * (iw + 2 * P) * (ih + 2 * P)) + j * S * (iw + 2 * P) + k * S;
                    for (size_t a = 0; a < f; ++a) {
                        for (size_t b = 0; b < f; ++b) {
                            size_t input_index = loc_input + a * D * (iw + 2 * P) + b * D;
                            size_t output_index = (l * f * f + a * f + b) * ow * oh * B + loc_output;
                            if (output_index < temp2.size() && input_index < temp1.size()) {
                                temp2[output_index] = temp1[input_index];
                            }
                        }
	...
	// print_vector(weights, "FLOAT", "weights:", 9*3*4);
	// print_vector(biases, "FLOAT", "biases:", 12);
}
